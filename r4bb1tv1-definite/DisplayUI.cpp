...
DisplayUI::DisplayUI() : batteryMonitor(A0) {}
...
void DisplayUI::drawBatteryIconWithPercentage() {
    // Verifica se o menu atual é o menu principal antes de desenhar a barra de bateria
    if (currentMenu == &mainMenu) {
        int percentage = batteryMonitor.getBatteryPercentage(); // Obtém a porcentagem da bateria

        // Configurações da barra de bateria
        int barWidth = 25; // Largura total da barra de bateria
        int barHeight = 10; // Altura da barra de bateria
        int x = 128 - barWidth - 2; // Posição x da barra (2 pixels de margem à direita)
        int y = 0; // Posição y da barra (no topo)

        // Define a cor para desenhar
        display.setColor(WHITE);

        // Desenha o contorno da bateria
        display.drawRect(x, y, barWidth, barHeight);

        // Calcula a largura do preenchimento baseado na porcentagem da bateria
        int fillWidth = (percentage * (barWidth - 2)) / 100; // Subtrai 2 para a borda

        // Desenha o preenchimento da bateria
        display.fillRect(x + 1, y + 1, fillWidth, barHeight - 2);

        // Desenha o indicador da bateria (parte superior)
        display.fillRect(x + barWidth, y + barHeight / 4, 2, barHeight / 2);

        // Desenha a porcentagem da bateria ao lado da barra (se necessário)
        //String batText = String(percentage) + "%";
        //display.drawString(x - 6 * batText.length() - 2, y, batText); // 6 pixels por caractere e 2 pixels de margem
...
void DisplayUI::draw(bool force) {
    if (force || ((currentTime - drawTime > drawInterval) && currentMenu)) {
        ...
        switch (mode) {
            ...
            case DISPLAY_MODE::MENU:
                drawMenu();
                drawBatteryIconWithPercentage();
                //drawBatteryPercentage(); // Chama o método para desenhar a porcentagem da bateria na tela do menu
                break;
